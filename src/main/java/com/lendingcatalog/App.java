...
//        try {
            //Splitting members.dat at delimiter
            List<String> membersData = FileStorageService.readContentsOfFile(FILE_BASE_PATH + "members.dat");
            for (String memberContents : membersData) {
                String[] splitValues = memberContents.split(FIELD_DELIMITER);
                String firstName = splitValues[0];
                String lastName = splitValues[1];
                String fileName = splitValues[2];
                //TODO-check length if/else
                //Reading fileName and categorizing
                List<String> memberItems = FileStorageService.readContentsOfFile(FILE_BASE_PATH + fileName);
                List<CatalogItem> catalogItemList = parseCatalogItems(memberItems);
                //Put member and their items into Map on line 19
...
//        }
    ...
    private List<CatalogItem> parseCatalogItems( List<String> memberItems){
        //Parsing Data into Catalog Map
        List<CatalogItem> catalogItemList = new ArrayList<>();
        for (String itemContents : memberItems) {
            String[] splitContents = itemContents.split(FIELD_DELIMITER);
            String category = splitContents[0];
            try {
            if (category.equals("book")) {
                String title = splitContents[1];
                String author = splitContents[2];
                LocalDate pubDate = LocalDate.parse(splitContents[3]);
                Book book = new Book(title, author, pubDate);
                book.registerItem();
                catalogItemList.add(book);
            } else if (category.equals("movie")) {
                String name = splitContents[1];
                String director = splitContents[2];
                LocalDate releaseDate = LocalDate.parse(splitContents[3]);
                Movie movie = new Movie(name, director, releaseDate);
                movie.registerItem();
                catalogItemList.add(movie);
            } else if (category.equals("tool")) {
                String type = splitContents[1];
                String manufacturer = splitContents[2];
                int count = Integer.parseInt(splitContents[3]);
                Tool tool = new Tool(type, manufacturer, count);
                tool.registerItem();
                catalogItemList.add(tool);
              } else{
